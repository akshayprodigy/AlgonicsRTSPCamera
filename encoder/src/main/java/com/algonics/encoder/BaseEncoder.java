...
public abstract class BaseEncoder implements EncoderCallback {
        ...
        public void run() {
          while (running) {
            try {
              ...
              Log.i(TAG, "Server Encoding IllegalStateException", e);
              reloadCodec();
            }catch (Exception e){
              Log.i(TAG, "Server Encoding error", e);
            }
          }
        }
      });
  ...
  protected void getDataFromEncoder() throws IllegalStateException {
    ...
    while (running) {
      ...
      Log.e("RtspServer","Server outBufferIndex: "+outBufferIndex +" codec: "+codec);
  ...
  private void createAsyncCallback() {
    callback = new MediaCodec.Callback() {
      ...
      public void onOutputFormatChanged(@NonNull MediaCodec mediaCodec,
        ...
        //Log.d("RtspServerCamera2", "Server "+Log.getStackTraceString(new Exception()));
        //Log.d("RtspServerCamera2", "Server "+ Log.getStackTraceString(Exception()))
        //Log.e("RtspServer","onOutputFormatChanged: mediaFormat "+mediaFormat +" mediaCodec: "+mediaCodec);
