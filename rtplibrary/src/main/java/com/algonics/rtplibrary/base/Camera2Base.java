package com.algonics.rtplibrary.base;
...
import com.algonics.encoder.Frame;
import com.algonics.encoder.audio.AudioEncoder;
import com.algonics.encoder.audio.GetAacData;
import com.algonics.encoder.input.audio.CustomAudioEffect;
import com.algonics.encoder.input.audio.GetMicrophoneData;
import com.algonics.encoder.input.audio.MicrophoneManager;
import com.algonics.encoder.input.audio.MicrophoneManagerManual;
import com.algonics.encoder.input.audio.MicrophoneMode;
import com.algonics.encoder.input.video.Camera2ApiManager;
import com.algonics.encoder.input.video.CameraCallbacks;
...
import com.algonics.encoder.video.FormatVideoEncoder;
import com.algonics.encoder.video.GetVideoData;
import com.algonics.encoder.video.VideoEncoder;
import com.algonics.rtplibrary.util.CongetionDataOfAllClients;
import com.algonics.rtplibrary.util.FpsListener;
import com.algonics.rtplibrary.util.RecordController;
import com.algonics.rtplibrary.view.GlInterface;
import com.algonics.rtplibrary.view.LightOpenGlView;
import com.algonics.rtplibrary.view.OffScreenGlThread;
...
public abstract class Camera2Base implements GetAacData, GetVideoData, GetMicrophoneData {
  ...
  public boolean prepareVideo(int width, int height, int fps, int bitrate, int iFrameInterval,
    ...
    //Log.e("RtspServer","Server started  prepareVideo");
    ...
    //Log.e("RtspServer","Server started  prepareVideo result "+result);
    return result;
  }
   ...
   *
   * @param cameraFacing front or back camera. Like: {@link com.algonics.encoder.input.video.CameraHelper.Facing#BACK}
   * {@link com.algonics.encoder.input.video.CameraHelper.Facing#FRONT}
   * @param rotation camera rotation (0, 90, 180, 270). Recommended: {@link
   * com.algonics.encoder.input.video.CameraHelper#getCameraOrientation(Context)}
   */
  ...
  public void startStream(String url) {
    ...
    if (!recordController.isRunning()) {
      Log.e("RtspServer","Server startEncoders: ");
      startEncoders();
    } else {
      Log.e("RtspServer","Server resetVideoEncoder: ");
      resetVideoEncoder(true);
    }
    Log.e("RtspServer","Server startStream url: "+url);
    startStreamRtp(url);
    onPreview = true;
  }
  ...
  private void startEncoders() {
    ...
    if (glInterface == null && !cameraManager.isRunning() && videoEncoder.getWidth() != previewWidth
      ...
      Log.e("RtspServer","Server startStream cameraManager.openLastCamera ");
      cameraManager.openLastCamera();
    }
    onPreview = true;
  }
  ...
  //cache control
  public abstract CongetionDataOfAllClients hasCongestion();
  ...
  private void prepareCameraManager() {
    ...
    //Log.e("RtspServer","Server started prepareCameraManager");
    if (textureView != null) {
      cameraManager.prepareCamera(textureView, videoEncoder.getInputSurface(),
          videoEncoder.getFps());
     // Log.e("RtspServer","Server started textureView");
    } else if (surfaceView != null) {
      cameraManager.prepareCamera(surfaceView, videoEncoder.getInputSurface(),
          videoEncoder.getFps());
      //Log.e("RtspServer","Server started surfaceView");
    } else if (glInterface != null) {
      Log.e("RtspServer","Server started glInterface");
    } else {
      cameraManager.prepareCamera(videoEncoder.getInputSurface(), videoEncoder.getFps());
      //Log.e("RtspServer","Server started videoEncoder");
    }
    videoEnabled = true;
  }
  ...
  public void onSpsPps(ByteBuffer sps, ByteBuffer pps) {
    Log.i("Camera2", "Server onSpsPps"+ sps+" pps"+pps );
